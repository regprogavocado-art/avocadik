import {categories,getSettings,mapProduct,type Env,type Wallet} from './content.ts';
...
export async function mutate(action:string,request:Request,env:Env,form:FormData,_session:AdminSession):Promise<Response> {
  ...
  try {
    if(action==='pages') {
      ...
      await saveRecord(env,'cms_products',numberId(form),{slug:slug(form),title:field(form,'title',150,true),category:choice(form,'category',categories.map(c=>c.id)),summary:field(form,'summary',500),body:field(form,'body',30000),price,price_currency:currency,availability:choice(form,'availability',['available','on_request','soon']),country_codes:JSON.stringify([...new Set(codes)]),image:imagePath(field(form,'image',300)),status:choice(form,'status',['published','draft'])});
...
function table(headers:string[],rows:string[][]){return rows.length?`<div class="table-scroll"><table><thead><tr>${headers.map(h=>`<th scope="col">${escape(h)}</th>`).join('')}</tr></thead><tbody>${rows.map(cells=>`<tr>${cells.map(c=>`<td>${c}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`:'<p class="muted">Записей пока нет.</p>';}
...
export async function renderAdmin(section:string,env:Env,session:AdminSession,url:URL):Promise<{title:string;html:string}> {
  ...
  if(!section) {
    ...
    html=panel('Счета',table(['Услуга','Сумма','Статус','Создан',''],rows.results.slice(0,pageSize).map(r=>[escape(r.description),`${escape(r.amount)} ${escape(r.currency)}`,badge(r.status),escape(date(r.created_at)),edit(section,r.id)]))+pagination(section,page,rows.results.length>pageSize));

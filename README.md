# avocado.rest — мини-сайт, CMS и чат через Telegram

Новый сайт находится в [`site/`](site/README.md): Astro 7, Cloudflare Pages,
D1 для контента и R2 для изображений. В `/admin` — страницы, каталог, страны,
новости, обращения, ручные криптовалютные счета и настройки. Инструкция запуска,
проверок и выкладки — в **[site/README.md](site/README.md)**; порядок переноса домена —
в [MIGRATION.md](design-system/avocado/MIGRATION.md).

Исходный лендинг ниже остаётся на GitHub Pages до переключения DNS.
`site/`, `pages/`, `worker/` и документация исключены из Jekyll-публикации.
Токены, пароли, локальные базы, резервные копии и исходники рендеров не коммитятся.

Новости: бот принимает новые и изменённые посты разрешённого канала `@avocadodev`.
Подробности импорта, модерации и отложенной загрузки фотографий — в [worker/NEWS.md](worker/NEWS.md).

Avocado представлен прежде всего как разработчик ПО: Hass, Ettinger и заказная
разработка. Каталог инфраструктуры дополняет продукты. Логотип, большая сцена
и персонаж «Avocado на страже» описаны в [арт-дирекшене](design-system/avocado/brand-art-direction.md).
В CMS внесены предварительные партнёрские тарифы с источниками и датами проверки.
...
и reply на свои сообщения. Для ответов оператора используйте личный чат или группу;
новостной канал подключается отдельно через `NEWS_CHANNEL_ID`. Для группы с темами задайте `ADMIN_THREAD_ID`
